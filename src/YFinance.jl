module YFinance

    using Base64
    using OrderedCollections
    using Dates
    using Downloads
    using JSON
    using Tables
    using PrecompileTools: @setup_workload, @compile_workload

    # Public API — Data retrieval (new v0.3 names)
    export prices, dividends, splits
    export fundamentals, options
    export quote_summary
    export search_symbols, search_news
    export is_valid_symbol, valid_symbols

    # Public API — Data retrieval (legacy names, still work)
    export get_prices, get_dividends, get_splits
    export get_Fundamental, get_Options
    export get_quoteSummary
    export get_all_symbols, get_symbols
    export validate_symbol, get_valid_symbols

    # Public API — Types
    export YahooSearch, YahooSearchItem
    export YahooNews, NewsItem
    export YFinanceTable
    export titles, links, timestamps
    export sink_prices_to

    # Public API — Configuration (new v0.3 names)
    export set_proxy, clear_proxy

    # Public API — Configuration (legacy names)
    export create_proxy_settings, clear_proxy_settings

    # Public API — QuoteSummary convenience accessors (new v0.3 names)
    export calendar_events, earnings_estimates, eps
    export insider_holders, insider_transactions
    export institutional_ownership, major_holders_breakdown
    export recommendation_trend, summary_detail
    export sector_industry, upgrade_downgrade_history

    # Public API — QuoteSummary convenience accessors (legacy names)
    export get_calendar_events, get_earnings_estimates, get_eps
    export get_insider_holders, get_insider_transactions
    export get_institutional_ownership, get_major_holders_breakdown
    export get_recommendation_trend, get_summary_detail
    export get_sector_industry, get_upgrade_downgrade_history

    # Public API — Constants (for discoverability)
    export QUOTE_SUMMARY_ITEMS, FUNDAMENTAL_TYPES, FUNDAMENTAL_INTERVALS
    export _QuoteSummary_Items, _Fundamental_Types, _Fundamental_Intervals
    export _PROXY_SETTINGS

    # Load Order
    include("headers.jl");
    include("network.jl");
    include("cookie_and_crumb.jl");
    include("Proxy_Auth.jl");
    include("Validity.jl");
    include("Prices.jl");
    include("QuoteSummary.jl");
    include("Fundamental.jl");
    include("Options.jl");
    # include("ESG.jl");
    include("Search_Symbol.jl");
    include("News_Search.jl");
    include("tables.jl");
    include("aliases.jl");

    @setup_workload begin
        price_test_resp = UInt8[];
        _clean_prc_float = randn(5);
        _clean_prc_int = rand(Int, 5);
        _clean_prc_int_nothing = vcat(nothing,_clean_prc_float);
        _clean_prc_float_nothing = vcat(nothing,_clean_prc_int_nothing);
        @compile_workload begin
            _date_to_unix(Date(2000));
            _date_to_unix(DateTime(2000));
            _date_to_unix("2000-01-01");

            _clean_prices_nothing(_clean_prc_float);
            _clean_prices_nothing(_clean_prc_int);
            _clean_prices_nothing(_clean_prc_int_nothing);
            _clean_prices_nothing(_clean_prc_float_nothing);

            _process_response(price_test_resp, "AAPL", "1d", false, false,false);
            _process_response(price_test_resp, "AAPL", "1d", true, false,false);
            _process_response(price_test_resp, "AAPL", "1d", false, true,false);
            _process_response(price_test_resp, "AAPL", "1d", false, false,true);
            _process_response(price_test_resp, "AAPL", "1d", true,true,false);
            _process_response(price_test_resp, "AAPL", "1d", false,true,true);
            _process_response(price_test_resp, "AAPL", "1d", true, true,true);
        end
    end


end